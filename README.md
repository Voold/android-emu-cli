...
сначала ищет готовые `mitmproxy`, `mitmdump` и `mitmweb` текущего пользователя.
Если найден TCP-listener, утилита без дополнительного вопроса использует его
порт и не запускает второй proxy. При нескольких listener приоритет получает
сохранённый порт, затем минимальный. Если готового процесса нет, утилита
спрашивает, запустить ли mitmproxy: отказ запускает AVD напрямую только один раз
и не меняет настройки.

При новом MITM-запуске coordinator находит свободный порт и добавляет адрес
`http://127.0.0.1:<port>`. Такой динамический proxy никогда не сохраняется среди
emulator-флагов. Когда MITM выключен, можно задать свой proxy. До запуска утилита
спрашивает, сохранить ли выбранные параметры: только подтверждённый ответ
записывает флаги, proxy и AVD MITM-override.
...
стека. В этом же меню есть «Убить все процессы mitm»: после подтверждения
утилита завершает только процессы `mitmproxy`, `mitmdump` и `mitmweb` текущего
пользователя, сначала штатно, затем принудительно завершает оставшиеся.

Новые MITM-конфигурации по умолчанию запускаются с `--ssl-insecure`, чтобы
mitmproxy принимал self-signed и внутренние сертификаты upstream-серверов.
Это отключает проверку подлинности сертификата между mitmproxy и upstream для
всего сеанса. Флаг можно снять в «Готовых флагах», если нужна строгая проверка
серверных TLS-сертификатов.

При новом MITM-запуске открываются отдельные окна Terminal для интерактивного
mitmproxy и логов emulator. Если MITM уже работал, открывается только окно
эмулятора. Закрытие окна с логами emulator завершает устройство. Прямой запуск
также открывает отдельный Terminal с логами emulator.
